...
const updateEvent = async (
  ...
  const response = await abortablePutRequest<Event>(`events/${id}`, eventToEdit)
    .request;
  return response.data;
};

const getEventList = async () => {
  const response: AxiosResponse<Event[]> =
    await abortableGetRequest<Event[]>("events/list").request;
  return response.data;
};
